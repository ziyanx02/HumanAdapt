...
import torch
import wandb
from envs.reward_wrapper_gpt import RewardEnv
from envs.time_wrapper_gpt import TimeWrapper
...
def main(args):
    ...
    if args.resume_cfg != None:
        ...
        with open(f'./cfgs/{args.task}_gpt.yaml', 'r') as file:
    ...
    env.compute_critic_observation()
    num_obs = env.obs_buf.shape[1]
    num_critic_obs = env.privileged_obs_buf.shape[1]
    env.num_obs = num_obs
    env.num_privileged_obs = num_critic_obs
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--offline', action='store_true', default=True)
